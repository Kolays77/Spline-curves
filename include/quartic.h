...
//         2 real roots: x[0], x[1]=x[2]          return 2 
...
std::vector<std::complex<long double>>  solve_sqrt(int deg, std::complex<long double> num) {
	std::complex<long double> r = std::pow(std::abs(num), 1.0L/deg);
	long double phi = std::arg(num);
	std::vector<std::complex<long double>> res;
	for (int i = 0 ; i < deg; ++i) {
		res.push_back(r * std::complex<long double>(std::cos((phi + 2 * PI * i) / deg), 
													std::sin((phi + 2 * PI * i) / deg)));
	}
	return res;
}
...
std::vector<std::complex<long double>> solveP4(long double a, long double b, long double c, long double d) {
	if (std::abs(a) < eps && std::abs(a) < eps && std::abs(a) < eps) {
